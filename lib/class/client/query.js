...
class Query {
	...
	stream = async (enable_interrogative_upspeak) => {
		const query_res = await this.#rest.post(routes.query({
			text: this.text,
			speaker: this.speaker,
		}), headers.json());
		const voice_res = await this.#rest.post(routes.synthesis({
			speaker: this.speaker,
			enable_interrogative_upspeak: enable_interrogative_upspeak ?? true,
		}), headers.wav(), await query_res.text());
		return voice_res.body;
	}
}
